...
- `10 0 * * *` → `/api/revalidate-today`, which regenerates the 240 hubs and then **verifies itself**: it fetches three sampled hubs, parses the calendar day out of their JSON-LD `Event` (`lib/hub-freshness.ts`), and returns 500 when the day is neither today's nor yesterday's, so a broken cron shows as a failed invocation instead of a cheerful `{revalidated: 240}`. Two days are allowed on purpose — it makes the verdict independent of the still-unresolved cache mechanism. The sample is `es/madrid`, `en/tokio`, `es/sidney` — three zones in three different relationships to UTC and the date line, so a zone-dependent fault shows in one probe and not the others. (It was originally chosen because the Event nodes dropped on a city's DST transition day and an all-European sample went blind twice a year; `sunEvent` now labels the transition day instead of dropping it, so that hole is closed and the spread is kept for the remaining reason.)
