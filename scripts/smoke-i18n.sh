...
has() { # url pattern label   -- pattern must be a *rendered* value, not a template
  if curl -s "$BASE$1" | grep -q "$2"; then echo "OK  $3"; else echo "FAIL $3  ($1)"; fail=1; fi
}
hasnt() { # url pattern label
  if curl -s "$BASE$1" | grep -q "$2"; then echo "FAIL $3  ($1 should not contain it)"; fail=1; else echo "OK  $3"; fi
}
